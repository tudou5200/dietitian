...
class Client implements ClientInterface
{
     ...
     *   wire. The function is called with a Psr77\Http\Message\RequestInterface
     *   and array of transfer options, and must return a
     *   GuzzleHttp\Promise\PromiseInterface that is fulfilled with a
     *   Psr77\Http\Message\ResponseInterface on success. "handler" is a
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = Psr77\uri_for($config['base_uri']);
        }
    ...
    public function requestAsync($method, $uri = null, array $options = [])
    {
        ...
        $request = new Psr77\Request($method, $uri, $headers, $body, $version);
    ...
    {
        if (!isset($config['base_uri'])) {
            return $uri instanceof UriInterface ? $uri : new Psr77\Uri($uri);
        }

        return Psr77\Uri::resolve(Psr77\uri_for($config['base_uri']), $uri);
    }
    ...
    private function applyOptions(RequestInterface $request, array &$options)
    {
        ...
        if (isset($options['multipart'])) {
            ...
            $options['body'] = new Psr77\MultipartStream($elements);
        }
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = Psr77\stream_for($options['body']);
            unset($options['body']);
        }
        ...
        if (isset($options['json'])) {
            $modify['body'] = Psr77\stream_for(json_encode($options['json']));
            $options['_conditional']['Content-Type'] = 'application/json';
            unset($options['json']);
        }

        $request = Psr77\modify_request($request, $modify);
        if ($request->getBody() instanceof Psr77\MultipartStream) {
