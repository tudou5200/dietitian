...
class Fileup{
         ...
         * $path=$up->up("上传组件的名字");
         * #########################################################################
         * $tarr = array('filepath'=>'data/images','maxsize'=>'100','allowmime'=>array('image/jpeg','image/png','image/gif','image/pjpeg'));
         * $upObj = new fileup($tarr); 
         * $filename = $upObj->up("logoimg");
         * if ($filename == '') { // 若文件名为空则输出错误信息
         * 	echo 'msg= '.$upObj->geterror();
         * }else {
         * 	$_POST['logoimg'] = $filename;
         * }		 
		 */
